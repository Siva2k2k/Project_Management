import { z } from 'zod';
import { ProjectType, RAGStatus, ProjectTrackingBy } from '../types';
...
const milestoneSchema = z.object({
  ...
  scope_completed: z.number().min(0).max(100).default(0),
...
export const createProjectSchema = z.object({
  ...
  estimated_effort: z.number().min(0, 'Estimated effort cannot be negative'),
  estimated_budget: z.number().min(0, 'Estimated budget cannot be negative'),
  estimated_resources: z.number().min(0, 'Estimated resources cannot be negative'),
  scope_completed: z.number().min(0).max(100).default(0),
...
export const updateProjectSchema = z.object({
  ...
  estimated_effort: z.number().min(0, 'Estimated effort cannot be negative').optional(),
  estimated_budget: z.number().min(0, 'Estimated budget cannot be negative').optional(),
  estimated_resources: z.number().min(0, 'Estimated resources cannot be negative').optional(),
  scope_completed: z.number().min(0).max(100).optional(),
